...
class model_trainer():
    ...
    def init(self, model, tokenizer):
        ...
        model_name = "Llama2-7B-set-5" 
        
        # In this experiment we are training the model to predict whether protein folding will increase or decrease
        ...
        # Use 60 with small protein names for D1
        # Use 120 for long protein names
    ...
    def train_model(self):
        ...
        def compute_metrics(eval_preds):         
            ...
            for i, text in enumerate(decoded_texts):
                ...
                #print(f"decoded text for query {i+1}: ", decoded_texts)
                ...
                if "yes" in first_word.lower(): preds.append(1)
                    ...
                    if true_labels[i] == 0: preds.append(1)
                    else: preds.append(0)
    ...
    def inference(self):
        ...
        with torch.no_grad():
            ...
            for example in self.test_dataset:
                ...
                generated_sequences = self.model.generate(**inputs, max_new_tokens=self.new_tokens)

                # Decode the model's output
                decoded_output = self.tokenizer.decode(generated_sequences[0], skip_special_tokens=True).lower().strip()
                print("Generated Sequence: ", decoded_output)
                #self.log(f"Generated Sequence: {decoded_output}")
                
                # Calculate the new tokens generated
                input_length = inputs['input_ids'].shape[1]

                # Directly decode the new tokens generated by slicing the generated_sequences based on input_length
                new_tokens_generated = self.tokenizer.decode(generated_sequences[0][input_length:], skip_special_tokens=True).lower().strip()
                # print("New Tokens Generated: ", new_tokens_generated)
                # self.log(f"New Tokens Generated: {new_tokens_generated}")

                # Process the output and extract the first 'yes' or 'no'
                #matches = re.findall(r'yes,|yes |yes.|yes|no,| no|no |no.', new_tokens_generated)
                matches = re.findall(r'\byes\b|\bno\b', new_tokens_generated)
                first_word = matches[0] if matches else ''

                # Determine true label based on decoded label
                if "yes" in true_answer: true_label = 1
                elif "no" in true_answer: true_label = 0
                else: print("Error: ", true_answer)

                # # Create binary predictions based on the model's text output
                if "yes" in first_word: prediction = 1
                elif "no" in first_word: prediction = 0
                else:
                    # Append the opposite value of x 
                    print("Model Unsure, marking as incorrect")
                    if true_label == 0: prediction = 1
                    else: prediction = 0
