...
class BioMedLMPrompter:
	...
	def get_string_prompt(self):
		...
		#entity_q = lambda entity: "Question: Which proteins are related to {x}? Answer:".format(x=entity)
		...
		relation_q = lambda e1, e2: "Question: Do {x} and {y} interact with each other? Answer:".format(x=e1, y=e2)
	...
	def get_kegg_prompt(self):
		entity_q = lambda pathway: "Question: Which genes are involved in \"{x}\"? Answer:".format(x=pathway)
		...
		relation_q = lambda e1, e2: "Question: Is the gene \"{x}\" involved in the pathway \"{y}\"? Answer:".format(x=e1, y=e2)
		relation_a = lambda answer: " {x}\n\n".format(x=answer)
	...
	def get_indra_prompt(self):
		...
		relation_type_q = lambda e1, e2, choices: "\n\nQuestion: Given the options: {z}, which one is the relation type between {x} and {y} in the text above? Answer:".format(x=e1, y=e2, z=choices)
		...
		#relation_type_q = lambda e1, e2, text, choices: "Question: Given the options: {z}, which one is the relation type between {x} and {y}? Context: \"{t}\"".format(x=e1, y=e2, t=text, z=choices)
