...
"""
import os
os.environ["HF_HOME"] = "/hpcgpfs01/work/sjantre/Bayesian_lora/bayesian_lora/.cache/huggingface/"
import logging
import torch as t
...
def variance(
    ...
    for k, (A, S) in factors.items():
        ...
        if G.ndim == 3:  # Check if batch dimension is missing when batch size is 1
            G = G.unsqueeze(0)
        if G.shape[-1] != n_lora:
            G = G.mT
        ...
        # print(term_1.shape)
        # print(batch_size, n_logits, n_logits)
